...
pages.forEach((page) => {
  ...
  assert.ok(html.includes('script.js?v=stability-6'), `${page} should cache-bust optimized scripts`);
});
...
assert.ok(script.includes('navigator.serviceWorker.register("sw.js?v=click-stage-8")'), "shared script should register the versioned service worker");
