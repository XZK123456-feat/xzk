...
pages.forEach((page) => {
  ...
  assert.ok(
    html.includes('href="click-stage.css?v=click-stage-7"'),
    `${page} should request click-stage.css with the current stage release`,
  );
  assert.ok(
    html.includes('src="click-stage.js?v=click-stage-7"'),
    `${page} should request click-stage.js with the current stage release`,
  );
  assert.ok(
    html.includes('src="script.js?v=stability-5"'),
...
assert.ok(
  sharedSource.includes('navigator.serviceWorker.register("sw.js?v=click-stage-7")'),
  "the shared controller should register the current service worker release",
);
assert.ok(
  serviceWorkerSource.includes('const CACHE_VERSION = "zk-portfolio-click-stage-7"'),
  "the service worker should use the current stage cache generation",
);
[
  "./script.js?v=stability-5",
  "./click-stage.css?v=click-stage-7",
  "./click-stage.js?v=click-stage-7",
  "./home-stage.js?v=click-stage-2",
  "./detail-stage.js?v=click-stage-7",
